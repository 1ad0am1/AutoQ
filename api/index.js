...
function productOut(r){return {id:r.id,name:r.name,category:r.category,brand:r.brand||'',model:r.model||'',part_number:r.part_number||'',price:Number(r.price||0),stock:Number(r.stock||0),image:r.image||'',created:r.created,updated:r.updated};}
function orderOut(r){
...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET' && path==='/api/products'){
      ...
      return json(res,200,{products:rows.map(productOut)});
    }

    if(req.method==='POST' && path==='/api/admin/products'){
      const a=auth(req,res,'admin'); if(a.error)return;
      const b=req.body||{}; const name=String(b.name||'').trim(); const category=String(b.category||'').trim();
      const allowed=['german','japan','korea','china','oils'];
      const price=Number(b.price||0), stock=Math.max(0,parseInt(b.stock??0,10)||0);
      if(!name||!allowed.includes(category)||!Number.isFinite(price)||price<0)return json(res,400,{error:'اسم المنتج والقسم والسعر مطلوبة'});
      const rows=await sql`INSERT INTO products(name,category,brand,model,part_number,price,stock,image) VALUES(${name},${category},${String(b.brand||'')},${String(b.model||'')},${String(b.part_number||'')},${price},${stock},${String(b.image||'')}) RETURNING *`;
      return json(res,201,{product:productOut(rows[0])});
    }

    const pmatch=path.match(/^\/api\/admin\/products\/(\d+)$/);
    if(req.method==='PATCH' && pmatch){
      const a=auth(req,res,'admin'); if(a.error)return; const id=Number(pmatch[1]); const b=req.body||{};
      const allowed=['german','japan','korea','china','oils'];
      const name=String(b.name||'').trim(), category=String(b.category||'').trim(); const price=Number(b.price||0), stock=Math.max(0,parseInt(b.stock??0,10)||0);
      if(!name||!allowed.includes(category)||!Number.isFinite(price)||price<0)return json(res,400,{error:'بيانات المنتج غير صحيحة'});
      const rows=await sql`UPDATE products SET name=${name},category=${category},brand=${String(b.brand||'')},model=${String(b.model||'')},part_number=${String(b.part_number||'')},price=${price},stock=${stock},image=${String(b.image||'')},updated=NOW() WHERE id=${id} RETURNING *`;
      if(!rows.length)return json(res,404,{error:'المنتج غير موجود'}); return json(res,200,{product:productOut(rows[0])});
    }
    if(req.method==='DELETE' && pmatch){
      const a=auth(req,res,'admin'); if(a.error)return; const id=Number(pmatch[1]);
      const rows=await sql`DELETE FROM products WHERE id=${id} RETURNING id`; if(!rows.length)return json(res,404,{error:'المنتج غير موجود'});
